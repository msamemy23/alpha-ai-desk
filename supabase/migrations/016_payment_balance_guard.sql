create or replace function public.prevent_direct_payment_balance_change()
returns trigger
language plpgsql
security definer
...
revoke all on function public.prevent_direct_payment_balance_change() from public, anon, authenticated;

drop trigger if exists documents_amount_paid_guard on public.documents;
create trigger documents_amount_paid_guard
before update of amount_paid on public.documents
for each row execute function public.prevent_direct_payment_balance_change();
