...
async function transcribeViaOpenRouter(downloadUrl: string): Promise<string | null> {
  try {
    ...
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'openai/gpt-4o-mini-audio-preview', messages: [{ role: 'user', content: [{ type: 'input_audio', input_audio: { data: base64Audio, format: 'wav' } }, { type: 'text', text: 'Transcribe this audio recording word for word. Return ONLY the transcription text, nothing else.' }] }], max_tokens: 4000 }),
    })
...
async function scoreLeadFromTranscript(t: string): Promise<{lead_score:string;lead_reasoning:string;service_needed:string;caller_sentiment:string;key_quotes:string}> {
  ...
  if (!OPENROUTER_API_KEY||!t||t.length<20) return {...empty,lead_reasoning:'Transcript too short'}
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Authorization':`Bearer ${OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':'https://alpha-ai-desk.vercel.app','X-Title':'Alpha AI Desk'},body:JSON.stringify({model:AI_MODEL,messages:[{role:'system',content:'You are an AI for Alpha International Auto Center (Houston TX auto shop). Analyze call transcripts and return JSON only with these fields: lead_score (hot/warm/cold), lead_reasoning (1-2 sentences), service_needed, caller_sentiment (positive/neutral/frustrated/spam), key_quotes. Hot=ready to book. Warm=interested. Cold=price shopping/spam.'},{role:'user',content:`Analyze this call transcript and return JSON:\n\n${t.substring(0,3000)}`}],temperature:0.1,max_tokens:600,response_format:{type:'json_object'}})})
    if (!r.ok) return {...empty,lead_reasoning:`AI error: ${r.status}`}
...
async function getTelnyxTranscription(recordingId: string): Promise<{text:string|null;error?:string}> {
  try {
    const r = await fetch(`${TELNYX_BASE}/recordings/${recordingId}/transcriptions`,{headers:{'Authorization':`Bearer ${TELNYX_API_KEY}`}})
...
async function requestTelnyxTranscription(recordingId: string): Promise<boolean> {
  try {
    const r = await fetch(`${TELNYX_BASE}/recordings/${recordingId}/transcriptions`,{method:'POST',headers:{'Authorization':`Bearer ${TELNYX_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({language_code:'en-US'})})
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'debug') {
      ...
      return NextResponse.json({id:calls[0].id,has_recording_id:!!rd?.recording_id,recording_id:rd?.recording_id,has_mp3:!!rd?.download_urls?.mp3,has_wav:!!rd?.download_urls?.wav,has_openrouter_key:!!OPENROUTER_API_KEY,ai_model:AI_MODEL,raw_data_keys:rd?Object.keys(rd):[]})
    }
    if (action === 'score') {
      const {data:calls} = await db.from('call_history').select('id,transcript').not('transcript','is',null).neq('transcript','[transcription_failed]').is('lead_score',null).limit(limit)
      if (!calls?.length) return NextResponse.json({success:true,scored:0})
      let scored = 0
      for (const call of calls) {
        if (!call.transcript) continue
        const s = await scoreLeadFromTranscript(call.transcript)
        await db.from('call_history').update({lead_score:s.lead_score,lead_reasoning:s.lead_reasoning,service_needed:s.service_needed,caller_sentiment:s.caller_sentiment,key_quotes:s.key_quotes}).eq('id',call.id)
        scored++
      }
    ...
    if (action === 'batch') {
      ...
      for (const call of calls) {
        ...
        if (wavUrl) { transcript = await transcribeViaOpenRouter(wavUrl) }
        if (!transcript && recordingId) {
          const {text,error:telErr} = await getTelnyxTranscription(recordingId)
          if (text) transcript = text
          else transcriptError = telErr
        }
        if (!transcript && !wavUrl && !recordingId) { results.push({id:call.id,status:'no_audio_source'}); continue }
        if (!transcript) {
