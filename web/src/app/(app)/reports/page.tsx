...
export default function ReportsPage() {
  ...
  const [tab, setTab] = useState<'overview'|'revenue'|'jobs'|'calls'>('overview')
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in">
      ...
      <div className="flex gap-1 mb-5 bg-bg-card border border-border rounded-lg p-1 w-fit">
        {(['overview','revenue','jobs','calls'] as const).map(t => (
          <button key={t} className={`px-4 py-1.5 rounded text-sm font-medium capitalize transition-colors ${tab === t ? 'bg-blue text-white' : 'text-text-muted hover:text-text-primary'}`} onClick={() => setTab(t)}>
            {t === 'overview' ? 'Overview' : t === 'revenue' ? 'Revenue' : t === 'jobs' ? 'Jobs' : 'Calls'}
          </button>
        ))}
      </div>
