...
WEB SEARCH — Search the web for real-time information including prices, images, news, part numbers, and anything else. Use this whenever the user asks to "look online", "search for", "find prices", "find images", "find pictures", "show me", "check availability", "what does X cost", "go to google", "get me a picture of", or any request for current pricing/info/images. NEVER guess prices — always search first. ALWAYS use webSearch for images and Google requests — do NOT use webAutomate for Google or image searches:
...
export default function AIPage() {
  ...
  const agentLoop = async (history: ChatMessage[], featureFlags?: { search: boolean; webAutomation: boolean; socialMedia: boolean }) => {
    ...
    for (let step = 0; step < 10; step++) {
      ...
      if (parsed.tool === 'webAutomate') { const targetUrl = (parsed.url || parsed.view || '') as string; const isGoogleUrl = /google\.com|google\.co|images\.google|goo\.gl/i.test(targetUrl); const isImageSearch = /image|photo|picture|pikachu|pokemon|logo/i.test(targetUrl + ' ' + JSON.stringify(parsed)); if (isGoogleUrl || (parsed.action !== 'steel_session' && isImageSearch)) { agentMessages.push({ role: 'assistant', content: raw }); agentMessages.push({ role: 'user', content: 'IMPORTANT: Do NOT use webAutomate for Google or image searches. Use the webSearch tool instead with a relevant query. Try again with webSearch.' }); continue; } setStatus('Automating...'); let ar = ''; try { const r = await fetch('/api/web-automation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(parsed) }); const d = await r.json(); ar = d.ok ? `Success: ${JSON.stringify(d.data).slice(0, 500)}` : `Failed: ${d.error}` } catch (e) { ar = `Error: ${e instanceof Error ? e.message : 'Unknown'}` } accumulated.push(`[webAutomate]: ${ar}`); agentMessages.push({ role: 'assistant', content: raw }); agentMessages.push({ role: 'user', content: `Web automation result: ${ar}
