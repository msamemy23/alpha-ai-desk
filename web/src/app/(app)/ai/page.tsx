  ...
  const agentLoop = async (history: ChatMessage[], featureFlags?: { search: boolean; webAutomation: boolean; socialMedia: boolean }) => {
    ...
    for (let step = 0; step < 10; step++) {
      ...
      if (d.ok && d.data?.sessionViewerUrl) {
        ...
        const iframeHtml = `<div style="border:1px solid #333;border-radius:12px;overflow:hidden;margin:8px 0;background:#1a1a2e"><div style="padding:8px 12px;background:#1a1a2e;border-bottom:1px solid #333;display:flex;align-items:center;gap:8px"><span style="width:8px;height:8px;border-radius:50%;background:#10b981;display:inline-block;animation:pulse 2s infinite"></span><span style="color:#e2e8f0;font-size:13px;font-weight:600">Live Browser</span><a href="${viewerUrl}" target="_blank" style="margin-left:auto;color:#60a5fa;font-size:11px;text-decoration:none">Open in new tab ↗</a></div><iframe src="${viewerUrl}" style="width:100%;height:500px;border:none;background:#0f0f1a" sandbox="allow-same-origin allow-scripts allow-popups allow-forms" allow="clipboard-read;clipboard-write"></iframe></div>`;
