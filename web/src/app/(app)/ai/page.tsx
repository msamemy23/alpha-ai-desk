...
const CONNECTOR_SERVICE_INFO: Record<string, { icon: string; name: string; description: string; color: string; bgColor: string; oauthPath: string }> = {
  facebook: { icon: 'ðŸ“˜', name: 'Facebook Pages', description: 'Post updates, reply to comments, manage messages', color: '#1877F2', bgColor: 'rgba(24,119,242,0.1)', oauthPath: '/api/auth/facebook' },
  instagram: { icon: 'ðŸ“¸', name: 'Instagram Business', description: 'Post photos, reply to comments and DMs', color: '#E1306C', bgColor: 'rgba(225,48,108,0.1)', oauthPath: '/api/auth/facebook' },
  google_business: { icon: 'ðŸ—ºï¸', name: 'Google Business Profile', description: 'Post updates, reply to reviews, see ratings', color: '#4285F4', bgColor: 'rgba(66,133,244,0.1)', oauthPath: '/api/auth/google' },
  google_calendar: { icon: 'ðŸ“…', name: 'Google Calendar', description: 'Schedule appointments, manage bookings', color: '#0F9D58', bgColor: 'rgba(15,157,88,0.1)', oauthPath: '/api/auth/google' },
}
...
PERSONALITY: Confident, direct, knowledgeable. Short sentences. You know cars inside and out. Be conversational and natural â€” you're talking to a mechanic who's busy, be efficient.

SMART ASSISTANT RULES â€” YOU MUST FOLLOW THESE:

1. UNDERSTAND INTENT â€” THINK LIKE A MECHANIC:
...
- "look up parts for Rufina's car" = searchCustomers for Rufina â†’ get her vehicle â†’ search parts for that vehicle
- "how much for brakes on the Audi" = find the Audi in context â†’ search brake parts + prices
...
- "get all 4 brakes for alex rig phone 832-555-1234 and make an estimate with labor" = search alex rig â†’ find vehicle â†’ search parts â†’ calculate labor â†’ build full estimate

2. PARTS SEARCH FORMAT â€” ALWAYS USE THIS:
...
**Kit Option: PowerStop Front+Rear Kit** â€” $256.99 (includes pads) [View](real-url)

3. MULTI-STEP EXECUTION:
When one sentence implies multiple steps, do ALL of them automatically:
- Name mentioned â†’ searchCustomers FIRST â†’ get their vehicle
- "make an estimate" â†’ use prices already found + add labor
- "with labor" â†’ use standard labor times (see below)
- "send it" â†’ send via email/text without asking
...
- If the user says "use the $54.99 ones" â€” use $54.99, not a different price.
...
- Don't ask for confirmation on searches â€” just search
- Don't ask for the labor rate â€” it's always $120/hr
- Don't ask for tax rate â€” it's always 8.25% on parts
- If user says "flat" or "no tax" â€” set tax to 0
...
1. When the user mentions "look online", "search for", "find prices for", "look up parts", "check prices", "what does a ___ cost" â€” you MUST use the webSearch tool to find REAL prices. NEVER make up or estimate prices. NEVER guess part costs. Always search first.
...
3. When the user says "email it", "send it", "text it", "send the estimate", "email that to them" â€” take ACTION immediately. Use sendEstimateEmail or message tool. Don't ask for confirmation unless you're missing critical info (like the recipient).
...
6. When building estimates:
   ...
   - Search for part prices online before creating the estimate â€” use REAL prices
7. For quotes: search part prices first using webSearch, then proposeDocument with real numbers from the search

WHEN TO SEARCH vs WHEN TO BUILD AN ESTIMATE â€” THIS IS CRITICAL:
- If the user says "look online", "search for", "find prices", "give me options", "what's available", "check prices for" â†’ ONLY search and present results conversationally as plain text. Do NOT call proposeDocument. Do NOT create an estimate unless they explicitly ask.
- If the user says "make an estimate", "quote it", "build a quote", "make a receipt", "write it up", "create an estimate" â†’ THEN search prices first and use proposeDocument to create an estimate.
- If the user gives you specific line items with prices (like "Replace charcoal canister: $180, labor 1 hour") â†’ THEN they want an estimate, use proposeDocument.
- If the user says "look online for X AND make a quote/estimate" â†’ THEN search AND build an estimate.
...
- NOT everything needs to be an estimate. Sometimes the user just wants information. Be conversational FIRST â€” present info, then ask what they want to do next.
...
- ALWAYS include clickable markdown links to the source/product page for each result. The search results include URLs â€” use them.
...
TOOL CALLS â€” respond with ONLY a raw JSON object, no markdown, no code blocks, no extra text:

WEB SEARCH â€” Search the web for real-time information including prices, images, news, part numbers, and anything else. Use this whenever the user asks to "look online", "search for", "find prices", "find images", "find pictures", "show me", "check availability", "what does X cost", "go to google", "get me a picture of", or any request for current pricing/info/images. NEVER guess prices â€” always search first. Use webSearch for ALL web lookups. Use webSearch for ALL web lookups â€” prices, images, videos, part numbers, diagnostics, labor times, TSBs, recalls, and any real-time information. webSearch returns rich results including: text results with URLs and favicons, product images, YouTube videos with embed URLs, Price Radar (price comparison across stores with best deal highlighted), Knowledge Panels (DTC codes, fluid specs), AI summaries, Smart Follow-up suggestions, and Related Searches. webSearch now returns rich results including product images, YouTube videos, price comparisons (Price Radar), and smart follow-up suggestions.
...
CREATE CUSTOMER â€” Create a new customer record. Use when user mentions a new person's name/phone/email that isn't in the system yet:
{"tool":"action","action":"createCustomer","payload":{"name":"John Doe","phone":"555-1234","email":"john@example.com"}}

CREATE JOB â€” Open a new work order for a customer's vehicle:
...
CREATE DOCUMENT (visual preview card) â€” ALWAYS use proposeDocument for ALL document types: Invoices, Estimates, AND Receipts. This shows a formatted preview card with parts and labor breakdown so the user can review before saving. Include customer_email and customer_phone if you have them. Set "type" to "Invoice", "Estimate", or "Receipt" as appropriate:
...
EMAIL ESTIMATE/INVOICE â€” Send an estimate or invoice to a customer via email. Use when user says "email it", "send the estimate", "email invoice to John". Can look up by doc_number, customer_name, or customer_id:
...
SEND SMS TO CUSTOMER â€” Text an estimate or message to a customer. Use when user says "text it", "text the estimate", "send a text":
...
SEARCH CUSTOMERS â€” Search for existing customers by name, phone, email, or any text. ALWAYS use this BEFORE creating a customer. Use when user mentions ANY customer name, asks "find customer", "look up", "who is", "do we have", or any customer reference:
...
GET CUSTOMER HISTORY â€” Look up everything about a customer:
{"tool":"action","action":"getCustomerHistory","payload":{"customer_name":"John Doe"}}

GET SHOP STATS â€” Get current shop performance metrics:
...
PLACE PHONE CALL â€” connects the user directly to someone (you are NOT on the call):
{"tool":"call","to":"+15551234567","name":"Customer Name"}
Use when user says: "call 2819008141", "call John", "dial this number", "ring them" â€” just a number or name with no task attached.

AI VOICE CALL â€” AI calls someone and has a FULL CONVERSATION to complete a task (you ARE the caller):
...
- Keep it natural: "Got it â€” Paul Jones. What's his email and phone so I can add it to the estimate?"
...
- If the user says they don't have it or to skip it, that's fine â€” create the document without it.
- When proposing an estimate or creating an invoice, always pass customer_email and customer_phone if you have them. This is critical â€” never drop phone or email that the user provided.

EXECUTION RULES:
1. Think through the full plan before starting
2. Execute each step silently â€” ONE tool call per turn, no explanation text
3. NEVER output text AND a tool call together â€” pick one
4. For quotes/estimates: ALWAYS search part prices first (webSearch), then proposeDocument with REAL numbers. But ONLY use proposeDocument if the user actually asked for an estimate/quote â€” if they just asked to search or find prices, respond with plain text results instead.
...
6. When ALL steps done: respond in plain text â€” 1-3 sentences max confirming what was completed
7. NEVER ask for info already in the conversation â€” use what was provided
8. Confirm destructive actions (void, delete) before executing
9. When user says "email it" or "text it" â€” just do it, don't ask for confirmation unless you're missing the recipient
10. If a customer name and email are mentioned together, always associate them

SOCIAL MEDIA & CONNECTORS:
- When the user asks to post to Facebook, Instagram, or Google Business â€” use the connector tool
- When asked to check comments, reviews, or messages â€” fetch and present them
- When asked to reply to comments or reviews â€” use the reply connector tool
- When asked to schedule an appointment â€” use Google Calendar
- When asked to find customers on social media â€” search Facebook messages and comments
- Always confirm before posting publicly (show draft first, say "Here's the draft â€” want me to post this?")
- For Instagram posts, an image URL is required

CONNECTOR TOOL CALLS â€” respond with ONLY a raw JSON object:
...
export default function AIPage() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: "Hey! I'm Alpha AI â€” your shop's command center. I can create customers, open jobs, build estimates, send texts and emails, look up anyone's history, check shop stats, schedule follow-ups, and more. What do you need?" }
  ])
  ...
  // Voice mode: start listening via SpeechRecognition â€” sends through send()
  ...
  useEffect(() => {
    ...
    // Don't speak HTML-only messages (estimate cards etc) â€” speak content if available
  ...
  // Thinking timer â€” counts up while loading
  useEffect(() => {
    if (loading) {
      ...
      reader.onload = async () => {
        try {
          ...
          if (pdfjsLib) {
            ...
            setAttachedFile({ name: file.name, content: `[PDF file: ${file.name} â€” ${(file.size/1024).toFixed(0)}KB. PDF text extraction unavailable.]`, type: 'text' })
          }
        } catch {
  ...
  const send = useCallback(async (overrideInput?: string) => {
    ...
    setLoading(true)
    // Build message content â€” include attached file if any
    ...
    // AI voice call detection â€” "AI call", "call [business] and order/ask", "have AI call"
    ...
    if (aiCallMatch || /\bcall\b.*\b(?:autozone|napa|oreilly|o'reilly|advance|pepboys|store|shop|dealership|dealer|supplier)\b/i.test(text)) {
      // Let the AI handle it â€” it will use aiVoiceCall tool
    } else {
      // Direct call detection â€” bypass AI for plain "call XXXXXXXXXX"
    }

    // Direct call detection â€” bypass AI for explicit call commands
    ...
    if (callMatch) {
      ...
      // If no instructions ("call X"), it's a personal call â€” no AI script
      const callTask = instructions
        ? instructions
        : '' // empty = no AI script, just connect the call
        if (callTask) {
          // AI voice call â€” has instructions, use make-call API
          ...
          // Personal/browser call â€” user talks via WebRTC softphone
...
7. RECEIPT TYPE: When user asks for a receipt, use proposeDocument with type Receipt not Invoice. When user asks for an invoice, use type Invoice. Receipt = proof of payment received. Invoice = bill for work done. Estimate = quote before work. The type field in proposeDocument MUST match exactly what the user asked for.` +
                    `\n\nNATURAL LANGUAGE INTELLIGENCE â€” YOU ARE SMART, ACT LIKE IT:
...
FEATURE TOGGLES (current state):\n- Web Search: ${activeFeatures.search ? 'ON' : 'OFF'}\n- Social Media: ${activeFeatures.socialMedia ? 'ON' : 'OFF'}\n- Deep Thinking: ${activeFeatures.thinking ? 'ON' : 'OFF'}\nIf a feature is OFF and the user tries to use it, tell them to enable the toggle at the bottom of the chat input.`
      ...
      const raw = data.choices?.[0]?.message?.content?.trim() || ''
            ...
            // OUTPUT SANITIZATION â€” detect repeating patterns and cut them off
      ...
      // No tool call = final answer â€” show to user
      if (!parsed) {
        let mediaHtml = ''
            if (lastSearchMedia) {
              ...
              if (vids.length > 0) {
                mediaHtml += '<div style="margin-top:12px"><strong>ðŸ“º Videos</strong></div>'
                for (const v of vids) {
      ...
      // Web Search â€” execute silently, feed result back to AI
        ...
        try {
          ...
          const d = await r.json()
            ...
            if (d.price_radar?.length) {
              searchResult += '\n\nPRICE COMPARISON (Price Radar):\n' + (d.price_radar as {store:string;price:number;url:string;shipping?:string}[]).map((p: {store:string;price:number;url:string;shipping?:string}) => `- ${p.store}: $${p.price.toFixed(2)} ${p.shipping || ''} â€” ${p.url}`).join('\n')
      ...
      // DB Action â€” execute silently, feed result back to AI
      ...
      // Propose Document â€” show visual estimate card
      ...
      // AI Voice Call â€” dials with bidirectional streaming, AI handles conversation
      ...
      // Place Phone Call â€” dials immediately via Telnyx
      if (parsed.tool === 'call') {
        // Browser call â€” dispatch phone:call event to PhoneWidget
      ...
      // Message â€” show draft confirmation card
      if (parsed.tool === 'message') {
        ...
        const assistantMsg: ChatMessage = {
          role: 'assistant',
          content: `Draft ${channel === 'email' ? 'email' : 'text'} ready â€” review below and hit Send.`
        }
        setMessages(prev => [...prev, assistantMsg])
        saveToHistory([...history, assistantMsg])
        return
      ...
      // Connector tools â€” Facebook, Instagram, Google Business, Google Calendar
...
Continue silently.` }); continue } // Unknown â€” treat as final response
      const assistantMsg: ChatMessage = { role: 'assistant', content: raw }
      setMessages(prev => [...prev, assistantMsg])
      speak(raw)
      saveToHistory([...history, assistantMsg])
  ...
  const renderVoiceSummary = (d: {summary?: string; transcript?: Array<{speaker:string;text:string}>; duration?: number; to?: string; recording_url?: string}): string => {
    ...
    return `<div style="border:1px solid #374151;border-radius:12px;padding:16px;background:var(--bg-card,#1a1a2e)">
      <div style="font-weight:700;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center">
        <span>ðŸ“ž AI Call Summary</span>
  ...
  const renderProposal = (parsed: Record<string, unknown>): string => {
    ...
    return `<div class="proposal-card" id="proposal-${encodedData.slice(0, 8)}">
      <div class="font-bold text-base mb-2">Proposed ${docType} â€” ${parsed.customer || ''}</div>
  ...
  const suggested = [
    ...
    "Open a new job for the white 2020 Honda Civic â€” AC not blowing cold",
    ...
    <div className="flex flex-col h-screen max-h-screen">
      <div className="p-3 sm:p-6 border-b border-border flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-text-muted mt-0.5 truncate">Full shop control Â· Create, update, search, message â€” all from here</p>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          >
            {speakEnabled ? 'ðŸ”Š' : 'ðŸ”‡'} Voice
          </button>
          {/* Talk to Alpha â€” voice conversation mode toggle */}
          <button
            onClick={() => {
              if (voiceActive) {
                closeVoiceMode()
            ...
            title={voiceActive ? 'Voice mode ON â€” tap to stop' : 'Start voice conversation'}
          >
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-80 bg-bg-card border-l border-border h-full overflow-y-auto p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowHistory(false)} className="btn btn-secondary btn-sm">âœ•</button>
            </div>
                ...
                <div>
                  {!showJumpIn ? (
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <button onClick={() => setShowJumpIn(false)} className="btn btn-secondary btn-sm text-xs">âœ•</button>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="p-4 border-t border-border">
        ...
        {attachedFile && (
          <div className="flex items-center gap-2 mb-2">
            <div className="flex items-center gap-1.5 bg-blue/10 border border-blue/30 rounded-full px-3 py-1 text-xs text-blue max-w-xs">
              <span>{attachedFile.type === 'image' ? 'ðŸ–¼ï¸' : 'ðŸ“Ž'}</span>
        ...
        <div className="flex gap-2 items-end">
          <textarea
            ...
            rows={2}
            placeholder="Ask anything â€” or tell me to create, update, message, search..."
            value={input}
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          ...
          <button
            ...
            title={features.search ? 'Web search ON â€” click to disable' : 'Web search OFF â€” click to enable'}
          >
          ...
          <button
            ...
            title={features.socialMedia ? 'Social media ON â€” click to disable' : 'Social media OFF â€” click to enable'}
          >
        ...
        {voiceActive && voiceStatus === 'listening' && <p className="text-xs mt-2 text-red-400 animate-pulse">Voice mode active â€” speak now</p>}
      </div>
      ...
      {showConnectorsPopup && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            {/* Toast inside popup */}
            {connectorToast && (
              <div className="mx-5 mt-4 px-4 py-2 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-sm">âœ… {connectorToast}</div>
            )}
            ...
            <div className="p-5">
              ...
              <div className="mt-5 p-3 rounded-xl bg-bg-hover border border-border text-xs text-text-muted">
                <span className="font-medium text-text-secondary">ðŸ’¡ Tip:</span> Facebook & Instagram share one OAuth flow. Google Business & Calendar share one Google login.
              </div>
            </div>
          </div>
        </div>
