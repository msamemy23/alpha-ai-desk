...
export const TOOLS: ToolDefinition[] = [
  ...
  },
  {
    name: 'repairSearch',
    agentId: 'repair',
    description: 'Run the Repair Workspace flow: exact vehicle confidence, deep manual tree search, source-backed operation lines, safety gates, estimate draft, procedure-card candidates, TSBs, recalls, diagrams, and specs.',
    permission: 'read',
    timeoutMs: 60000,
    retry: { attempts: 1, backoffMs: 0 },
    audit: true,
    requiresConfirmation: false,
    inputSchema: { query: 'string', vehicle: 'vin/year/make/model/engine/trim/drivetrain/transmission/brake package optional' },
    resultShape: { ok: 'boolean', data: 'normalized vehicle, confidence, operation lines, manual matches, safety profile, estimate draft, source cards, warnings' },
  },
  {
    name: 'openBrowser',
    agentId: 'desktop',
...
export const SKILLS: SkillDefinition[] = [
  ...
  },
  {
    ...
    ownerAgentId: 'repair',
    description: 'Identify the exact vehicle, deep-search free public manual trees, rank procedure/spec/diagram matches, create technician-review operation lines, and prepare a safe estimate draft without inventing specs or copied procedure text.',
    requiredTools: ['repairSearch'],
    successCriteria: ['Vehicle confidence is shown', 'deep manual matches are ranked', 'operation lines are structured', 'safety gates are listed', 'estimate draft never uses fake zero pricing', 'no unsupported specs are invented'],
