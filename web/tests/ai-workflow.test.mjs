...
test('complete research is converted into a review when the model loops instead of drafting', async () => {
  const h = harness([{ kind: 'ask', fields: ['vehicle_trim'], message: 'What engine/trim is this 2005 Accord?' }])
  h.deps.execute = async (action, payload, key) => {
    h.calls.push({ action, payload, key })
    if (action === 'lookupParts') return {
      ...
      data: {
        ...
        options: [{ parts: [
          { name: 'AutoZone Front Brake Pad Set', position: 'Front', store: 'AutoZone', price: 80, quantity: 1 },
          { name: 'AutoZone Front Brake Rotor', position: 'Front', store: 'AutoZone', price: 65, quantity: 2 },
          { name: 'AutoZone Rear Brake Pad Set', position: 'Rear', store: 'AutoZone', price: 70, quantity: 1 },
          { name: 'AutoZone Rear Brake Rotor', position: 'Rear', store: 'AutoZone', price: 55, quantity: 2 },
        ] }],
        laborGuidance: { operation: 'Replace brake pads and rotors on all four wheels', hours: 3, basis: 'standard_estimate' },
      },
    }
  ...
  const reply = await h.run('Make an invoice for Fallback QA: 2005 Honda Accord, all four brakes and rotors from AutoZone. It has rear disc brakes. Look it up.', 'fallback')
  assert.equal(reply.status, 'approval')
  assert.equal(reply.approval.payload.customer_name, 'Fallback QA')
  assert.equal(reply.approval.payload.parts.length, 4)
  assert.equal(reply.approval.payload.labors[0].hours, 3)
  assert.equal(h.calls.filter(call => call.action === 'lookupParts').length, 1)
})
