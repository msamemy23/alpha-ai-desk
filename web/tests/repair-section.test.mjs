...
test('repair presentation leads with a plain mechanic answer and direct action links', async () => {
  ...
  assert.equal(view.primaryActionLabel, 'Pick Exact Vehicle First')
  ...
  assert.match(view.mechanicSummary, /Pick the exact engine\/trim/i)
  assert.match(view.jobCard.sourceState, /manual|source|candidate/i)
...
test('repair UI exposes workflow tabs, manual reader, source stack, saved drafts, and estimate handoff', () => {
  ...
  assert.match(repairPage, /Easy Repair View/)
  ...
  assert.match(repairPage, /Exact Vehicle Choices/)
  assert.match(repairPage, /Repair Card/)
  assert.match(repairPage, /Ask Next/)
  assert.match(repairPage, /primaryActionLabel/)
  assert.match(repairPage, /Open The Info/)
  assert.match(repairPage, /Pick Exact Vehicle First/)
...
test('repair agent, tool, skill, router, and chat handoff are registered', () => {
  ...
  assert.match(aiPage, /Mechanic Answer/)
  assert.match(aiPage, /Exact Vehicle Choices/)
  assert.match(aiPage, /Repair Card/)
  assert.match(aiPage, /Ask Next/)
  ...
  assert.match(aiPage, /needs exact engine\/trim for exact diagrams/)
